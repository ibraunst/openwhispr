...
const DBUS_SERVICE_NAME = "com.customwhispr.App";
const DBUS_OBJECT_PATH = "/com/customwhispr/App";
const DBUS_INTERFACE = "com.customwhispr.App";

const KEYBINDING_PATH =
  "/org/gnome/settings-daemon/plugins/media-keys/custom-keybindings/customwhispr/";
...
class GnomeShortcutManager {
  ...
  _createInterfaceClass(dbusModule, callback) {
    class customWhisprInterface extends dbusModule.interface.Interface {
      constructor() {
    ...
    customWhisprInterface.configureMembers({
      methods: {
        Toggle: { inSignature: "", outSignature: "" },
      },
    });

    return customWhisprInterface;
  }
  ...
  async registerKeybinding(shortcut = "<Alt>r") {
    ...
    try {
      ...
      execFileSync(
        "gsettings",
        ["set", `${KEYBINDING_SCHEMA}:${KEYBINDING_PATH}`, "name", "customWhispr Toggle"],
        { stdio: "pipe" }
      );
      execFileSync(
        "gsettings",
