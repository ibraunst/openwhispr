...
class ClipboardManager {
  ...
  showAccessibilityDialog(testError) {
    ...
    if (isStuckPermission) {
      dialogMessage = `🔒 customWhispr needs Accessibility permissions, but it looks like you may have OLD PERMISSIONS from a previous version.

❗ COMMON ISSUE: If you've rebuilt/reinstalled customWhispr, the old permissions may be "stuck" and preventing new ones.

🔧 To fix this:
1. Open System Settings → Privacy & Security → Accessibility
2. Look for ANY old "customWhispr" entries and REMOVE them (click the - button)
3. Also remove any entries that say "Electron" or have unclear names
4. Click the + button and manually add the NEW customWhispr app
5. Make sure the checkbox is enabled
6. Restart customWhispr
...
Would you like to open System Settings now?`;
      ...
      dialogMessage = `🔒 customWhispr needs Accessibility permissions to paste text into other applications.
...
4. Add customWhispr to the list and check the box
5. Restart customWhispr
