...
export default function PersonalNotesView({
  ...
  // Auto-generate notes when the meeting transcript is ready, then export after completion
  const autoGeneratedRef = useRef(false);
  const isAutoGenerateRunningRef = useRef(false);
  const pendingExportTranscriptRef = useRef<string | null>(null);
  ...
  useEffect(() => {
    if (isMeetingRecording) {
      ...
      pendingExportTranscriptRef.current = null;
    ...
    pendingExportTranscriptRef.current = meetingTranscript;
    handleGenerateNotes();
  }, [isMeetingRecording, isMeetingLocalProcessing, meetingTranscript, handleGenerateNotes]);

  // Export transcript to file once AI generation completes — uses final note title as filename
  ...
  useEffect(() => {
    if (
      ...
      pendingExportTranscriptRef.current
    ) {
      isAutoGenerateRunningRef.current = false;
      const transcript = pendingExportTranscriptRef.current;
      pendingExportTranscriptRef.current = null;
      ...
      if (exportDirectory) {
        ...
        window.electronAPI.saveTranscriptToFile?.(transcript, filename, exportDirectory).catch(() => {});
      }
    }
    prevActionStateRef.current = actionProcessingState ?? "idle";
  }, [actionProcessingState, localTitle, activeNoteId, handleTitleChange]);
