...
export default function PersonalNotesView({
  ...
  useEffect(() => {
    ...
    // Export transcript to file immediately when recording ends — don't wait for AI
    const { exportDirectory, defaultExportFormat } = getSettings();
    if (exportDirectory) {
      const meetingTitle = calendarTitleRef.current;
      ...
      const safeTitle = (meetingTitle || localTitle)?.replace(/[/\\?%*:|"<>]/g, "-") || "meeting";
      const filename = `Meeting | ${safeTitle} | ${datePart}.${ext}`;

      const headerLines: string[] = [];
      headerLines.push(`# ${meetingTitle || safeTitle}`);
      ...
      if (meetingAttendeesJson) {
        try {
          ...
          if (attendees.length > 0) {
            const list = attendees.map((a) => a.name || a.email || "Unknown").join(", ");
            headerLines.push(`**Attendees:** ${list}`);
          }
        } catch {
          // malformed JSON — skip attendees
        }
      }
      headerLines.push("", "---", "");
  ...
  // Update note title once AI generation completes
  const prevActionStateRef = useRef<string>("idle");
  useEffect(() => {
    if (
      prevActionStateRef.current === "processing" &&
      actionProcessingState === "success" &&
      isAutoGenerateRunningRef.current
    ) {
      isAutoGenerateRunningRef.current = false;
  ...
  }, [actionProcessingState, activeNoteId, handleTitleChange]);
