...
export default function PersonalNotesView({
  ...
  const {
    ...
    isLocalProcessing: isMeetingLocalProcessing,
    isDiarizing: isMeetingDiarizing,
    transcript: meetingTranscript,
    diarizedTranscript: meetingDiarizedTranscript,
    diarizationError: meetingDiarizationError,
  ...
  const calendarStartTimeRef = useRef<string | null>(null);
  const calendarAttendeesRef = useRef<string | null>(null);
  // Track the last exported transcript file so we can overwrite it with speaker labels
  const lastExportRef = useRef<{ filename: string; directory: string; headerLines: string[] } | null>(null);
  ...
  const handleApplyEnhancement = useCallback(
    async (enhancedContent: string, prompt: string) => {
      if (!activeNoteId) return;
      if (isAutoGenerateRunningRef.current) {
        pendingEnhancedContentRef.current = enhancedContent;
      }
  ...
  const handleStartMeetingRecording = useCallback(async () => {
    ...
    calendarStartTimeRef.current = null;
    calendarAttendeesRef.current = null;
    meetingNoteIdRef.current = activeNoteId;
    diarizationNoteIdRef.current = activeNoteId;
    ...
    if (result?.event?.summary) {
      ...
      calendarStartTimeRef.current = result.event.start_time || null;
      calendarAttendeesRef.current = result.event.attendees || null;
  ...
  useEffect(() => {
    ...
    diarizationNoteIdRef.current = meetingRecordingRequest.noteId;
  ...
  // When speaker diarization completes, refresh from DB, re-export file, and show toast
  // (the hook already persisted the transcript to the DB via updateNote)
  useEffect(() => {
    if (!meetingDiarizedTranscript) return;
    initializeNotes();
    // Overwrite the exported file with speaker-labeled transcript
    if (lastExportRef.current) {
      const { filename, directory, headerLines } = lastExportRef.current;
      const fileContent = headerLines.join("\n") + meetingDiarizedTranscript;
      window.electronAPI.saveTranscriptToFile?.(fileContent, filename, directory).catch(() => {});
      lastExportRef.current = null;
    }
    toast({ title: "Speaker labels added to transcript" });
  }, [meetingDiarizedTranscript, toast]);

  useEffect(() => {
    if (!meetingDiarizationError) return;
    toast({ title: "Speaker identification failed", description: meetingDiarizationError ?? undefined, variant: "destructive" });
  }, [meetingDiarizationError, toast]);

  // Auto-generate notes when the meeting transcript is ready
  const autoGeneratedRef = useRef(false);
  const isAutoGenerateRunningRef = useRef(false);
  const pendingEnhancedContentRef = useRef<string | null>(null);
  useEffect(() => {
    if (isMeetingRecording) {
      autoGeneratedRef.current = false;
      isAutoGenerateRunningRef.current = false;
      pendingEnhancedContentRef.current = null;
      return;
    }
    if (isMeetingLocalProcessing || !meetingTranscript || autoGeneratedRef.current) return;
    autoGeneratedRef.current = true;

    if (!getSettings().useReasoningModel) {
      // AI off — export transcript directly with current title
      const { exportDirectory, defaultExportFormat } = getSettings();
      if (exportDirectory) {
        const finalTitle = calendarTitleRef.current || localTitle;
        const meetingStartTime = calendarStartTimeRef.current;
        const meetingAttendeesJson = calendarAttendeesRef.current;
        const ext = defaultExportFormat === "md" ? "md" : "txt";
        const timestampSource = meetingStartTime ? new Date(meetingStartTime) : new Date();
        const _d = timestampSource;
        const datePart = `${String(_d.getMonth() + 1).padStart(2, "0")}-${String(_d.getDate()).padStart(2, "0")}-${String(_d.getFullYear()).slice(2)}`;
        const safeTitle = (finalTitle || "meeting").replace(/[/\\?%*:|"<>]/g, "-");
        const filename = `${safeTitle} | ${datePart}.${ext}`;
        const headerLines: string[] = [];
        headerLines.push(`# ${finalTitle || safeTitle}`);
        headerLines.push(`**Date:** ${timestampSource.toLocaleString(undefined, { dateStyle: "medium", timeStyle: "short" })}`);
        let attendeeList = "";
        if (meetingAttendeesJson) {
          try {
            const attendees: { name?: string | null; email?: string | null }[] = JSON.parse(meetingAttendeesJson);
            if (attendees.length > 0) attendeeList = attendees.map((a) => a.name || a.email || "Unknown").join(", ");
          } catch { /* ignore */ }
        }
        headerLines.push(`**Attendees:** ${attendeeList || "—"}`);
        headerLines.push("", "---", "");
        const fileContent = headerLines.join("\n") + meetingTranscript;
        window.electronAPI.saveTranscriptToFile?.(fileContent, filename, exportDirectory).catch(() => {});
        lastExportRef.current = { filename, directory: exportDirectory, headerLines };
      }
      return;
    }

    isAutoGenerateRunningRef.current = true;
    handleGenerateNotes();
  }, [isMeetingRecording, isMeetingLocalProcessing, meetingTranscript, handleGenerateNotes, localTitle]);

  // Update note title once AI generation completes (or fails)
  const prevActionStateRef = useRef<string>("idle");
  useEffect(() => {
    const wasProcessing = prevActionStateRef.current === "processing";
    const isDone =
      actionProcessingState === "success" || actionProcessingState === "error";

    if (wasProcessing && isDone && isAutoGenerateRunningRef.current) {
      isAutoGenerateRunningRef.current = false;
      const generatedContent = pendingEnhancedContentRef.current;
      pendingEnhancedContentRef.current = null;

      let meetingTitle: string | undefined;
      if (actionProcessingState === "success") {
        const subjectMatch = generatedContent?.match(/\*\*Subject:\*\*\s*(.+)/);
        const aiSubject = subjectMatch?.[1]?.trim();
        meetingTitle = calendarTitleRef.current || aiSubject || undefined;
        if (meetingTitle && activeNoteId) {
          handleTitleChange(meetingTitle);
        }
      } else {
        // AI failed — still use calendar title if available
        meetingTitle = calendarTitleRef.current || undefined;
        if (meetingTitle && activeNoteId) {
          handleTitleChange(meetingTitle);
        }
      }

      // Export transcript to file now that we have the best available title
      const { exportDirectory, defaultExportFormat } = getSettings();
      if (exportDirectory && meetingTranscript) {
        const finalTitle = meetingTitle || localTitle;
        const meetingStartTime = calendarStartTimeRef.current;
        const meetingAttendeesJson = calendarAttendeesRef.current;
        const ext = defaultExportFormat === "md" ? "md" : "txt";
        const timestampSource = meetingStartTime ? new Date(meetingStartTime) : new Date();
        const _d = timestampSource;
        const datePart = `${String(_d.getMonth() + 1).padStart(2, "0")}-${String(_d.getDate()).padStart(2, "0")}-${String(_d.getFullYear()).slice(2)}`;
        const safeTitle = finalTitle?.replace(/[/\\?%*:|"<>]/g, "-") || "meeting";
        const filename = `${safeTitle} | ${datePart}.${ext}`;
        ...
        if (meetingAttendeesJson) {
          try {
            ...
            if (attendees.length > 0) {
              attendeeList = attendees.map((a) => a.name || a.email || "Unknown").join(", ");
            }
          } catch {
    ...
    prevActionStateRef.current = actionProcessingState ?? "idle";
  }, [actionProcessingState, activeNoteId, handleTitleChange, meetingTranscript, localTitle]);
        ...
        {editorNote ? (
          <>
            <NoteEditor
              ...
              isMeetingLocalProcessing={isMeetingLocalProcessing}
              isMeetingDiarizing={isMeetingDiarizing}
              meetingTranscript={meetingDiarizedTranscript || meetingTranscript}
              onStartMeetingRecording={isMeetingMode ? handleStartMeetingRecording : undefined}
              ...
              actionPicker={
                <ActionPicker
                  onRunAction={(action) => {
                    const transcript = meetingDiarizedTranscript || meetingTranscript || activeNote?.transcript;
