...
export default function UploadAudioView({ onNoteCreated, onOpenSettings }: UploadAudioViewProps) {
  ...
  const iscustomWhisprCloud =
    isSignedIn && cloudTranscriptionMode === "customwhispr" && !useLocalWhisper;
  ...
  // Mode detection
  const isByok = !useLocalWhisper && !iscustomWhisprCloud;
  ...
  if (file) {
    if (useLocalWhisper) {
      ...
      // Cloud (customWhispr) — user is always signed in here
  ...
  const getActiveModelLabel = (): string => {
    if (iscustomWhisprCloud) return t("notes.upload.customwhisprCloud");
    if (useLocalWhisper) {
      if (localTranscriptionProvider === "nvidia")
  ...
  const handleTranscribe = async () => {
    ...
    const useChunkProgress = iscustomWhisprCloud && isLargeFile;
  ...
  const getTranscribingLabel = (): string => {
    if (iscustomWhisprCloud) return t("notes.upload.transcribingCloud");
  ...
  const modeSelector = isSignedIn ? (
    <div className="flex items-center rounded-md border border-foreground/6 dark:border-white/6 bg-surface-1/30 dark:bg-white/[0.02] p-0.5 mb-3">
      ...
      >
        <Cloud size={11} />
        {t("notes.upload.customwhisprCloud")}
      </button>
      <button
        ...
        <div className="max-w-[320px] mx-auto">
          ...
          {state === "selected" && file && (
            <SelectedView
              ...
              iscustomWhisprCloud={iscustomWhisprCloud}
...
function SelectedView({
  ...
  return (
    <div style={{ animation: "float-up 0.3s ease-out" }}>
      ...
      {isLargeFile && !requiresUpgrade && !fileTooLarge && iscustomWhisprCloud && (
